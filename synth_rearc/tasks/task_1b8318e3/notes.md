...
2. For each singleton, take the nearest anchor first and compute its preferred halo cell from the singleton's offset to the `2x2` block:
   ...
   - when a point from beyond a corner is clamped onto that corner, it shifts
     inward along its chosen side if the perpendicular side is already occupied
3. If that preferred halo cell is already occupied, try the next anchor.

The inward corner shift is applied symmetrically for approaches from the left,
right, top, and bottom. That is why the summary hint was discarded rather than
used directly.
